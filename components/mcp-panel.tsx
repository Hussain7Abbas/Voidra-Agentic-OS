...
export function McpPanel({ workspaceId, request }: { workspaceId: string; request: RequestService }) {
  ...
  return <article className="panel settings-card mcp-card">
    ...
    <div className="compact-list mcp-connections" aria-label="MCP connections">{connections.map((connection) => <div key={connection.id} className="mcp-connection">
      <span><strong>{connection.name} · {connection.status}</strong><small>{connection.transport}{connection.serverInfo ? ` · ${connection.serverInfo.name}@${connection.serverInfo.version}` : ""}{connection.protocolVersion ? ` · ${connection.protocolVersion}` : ""}<br />{connection.fingerprint ? `Server pin ${connection.fingerprint.slice(0, 16)} · ` : ""}{connection.diagnostic}</small></span>
      ...
      {connection.status === "ready" && <div className="mcp-capabilities">
        ...
        <div className="compact-list" aria-label={`${connection.name} tools`}>{connection.capabilities.tools.map((tool) => <button key={tool.name} onClick={() => prepareTool(connection.id, String(tool.name))}><span><strong>{tool.title ?? tool.name}</strong><small>{tool.description}<br />Schema pin {tool.schemaDigest?.slice(0, 16) ?? "unavailable"}</small></span><span>Review call</span></button>)}</div>
