...
export const serviceRequestSchema = z.discriminatedUnion("operation", [
  ...
  z.object({
    ...requestIdentity,
    operation: z.literal("feature.list"),
    payload: z.object({}).strict(),
  }).strict(),
  z.object({
    ...requestIdentity,
    operation: z.literal("feature.set"),
    payload: z.object({ feature: z.enum(["skills", "routers", "headless", "catalog", "applications", "command-center", "component-artifacts"]), enabled: z.boolean() }).strict(),
  }).strict(),
  z.object({
    ...requestIdentity,
  ...
  z.object({
    ...
    payload: z.object({ focus: z.object({ baseId: z.union([z.literal("private"), z.uuid()]), documentId: z.string().min(1).max(128) }).strict().optional(), depth: z.number().int().min(0).max(5).optional(), tag: z.string().max(200).optional(), filterOnly: z.boolean().optional(), limit: z.number().int().min(1).max(1_000).default(500), offset: z.number().int().min(0).max(1_000_000).default(0) }).strict(),
  }).strict(),
  z.object({
    ...requestIdentity,
  ...
  z.object({ ...requestIdentity, operation: z.literal("router.list"), payload: z.object({}).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("router.suggest"), payload: z.object({ domain: z.string().max(2048).default("") }).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("router.update"), payload: z.object({ suggestionId: z.uuid(), preview: z.string().min(1).max(1_000_000) }).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("router.apply"), payload: z.object({ suggestionId: z.uuid() }).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("application.list"), payload: z.object({}).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("application.recommend"), payload: z.object({ query: z.string().max(200).default("") }).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("widget.list"), payload: z.object({}).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("widget.prepareAction"), payload: z.object({ widgetId: z.string().min(1).max(120), actionId: z.string().min(1).max(120), arguments: z.record(z.string(), z.unknown()).default({}) }).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("widget.approveAction"), payload: z.object({ actionId: z.uuid(), approved: z.boolean() }).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("microapp.list"), payload: z.object({}).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("microapp.register"), payload: z.object({ title: z.string().trim().min(1).max(120), icon: z.string().max(20), surfaces: z.array(z.enum(["dashboard", "dock", "detail"])).min(1).max(3), dataNeeds: z.array(z.string().max(200)).max(30), storage: z.enum(["none", "workspace-namespaced"]), actions: z.array(z.object({ id: z.string().regex(/^[a-z0-9][a-z0-9._-]{0,79}$/), label: z.string().trim().min(1).max(120), risk: z.enum(["read", "write", "commit"]) }).strict()).max(20) }).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("microapp.promote"), payload: z.object({ appId: z.uuid(), artifactId: z.uuid() }).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("run.timeline"), payload: z.object({ limit: z.number().int().min(1).max(500).default(200) }).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("layout.get"), payload: z.object({}).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("layout.update"), payload: z.object({ expectedRevision: z.uuid(), items: z.array(z.object({ id: z.enum(["applications", "calendar", "pulse", "attention", "skills", "routines"]), zone: z.enum(["left", "right"]), order: z.number().int().min(0).max(5), size: z.enum(["compact", "standard", "expanded"]), visible: z.boolean() }).strict()).length(6) }).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("layout.reset"), payload: z.object({}).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("search.global"), payload: z.object({ query: z.string().trim().min(1).max(500), limit: z.number().int().min(1).max(100).default(30) }).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("output.search"), payload: z.object({ query: z.string().max(1000).default(""), kind: z.enum(["markdown", "text", "image", "pdf", "component", "legacy-html", "directory", "binary"]).nullable().optional(), provider: z.string().max(200).nullable().optional(), tags: z.array(z.string().max(100)).max(20).default([]), limit: z.number().int().min(1).max(500).default(200) }).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("output.register"), payload: z.object({ path: z.string().min(1).max(2048), title: z.string().max(200).optional(), tags: z.array(z.string().max(100)).max(50).default([]) }).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("output.read"), payload: z.object({ artifactId: z.uuid() }).strict() }).strict(),
  z.object({
    ...requestIdentity,
    operation: z.literal("artifact.semanticReview"),
    payload: z.object({
      artifactId: z.uuid(),
      purpose: z.string().trim().min(1).max(1000),
      manifest: z.record(z.string(), z.unknown()),
      files: z.record(z.string().min(1).max(2048), z.string().max(1_000_000)).refine((value) => Object.keys(value).length <= 100, "Too many artifact source files"),
      ...
      staticReview: z.object({
        ...
        verdict: z.enum(["pass", "pass-with-low-risk-notes"]),
        findings: z.array(z.object({ severity: z.enum(["low", "medium", "high", "critical"]), category: z.string().max(200), file: z.string().max(2048), evidence: z.string().max(1000), remediation: z.string().max(1000) }).strict()).max(200),
        filesExamined: z.array(z.string().max(2048)).max(100),
      }).strict(),
    }).strict(),
  }).strict(),
  ...
  z.object({ ...requestIdentity, operation: z.literal("skill.export"), payload: z.object({ skillId: z.uuid() }).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("skill.import"), payload: z.object({ content: z.string().min(1).max(35_000_000) }).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("skill.validate"), payload: z.object({ skillId: z.uuid() }).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("skill.resource.list"), payload: z.object({ skillId: z.uuid() }).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("skill.resource.read"), payload: z.object({ skillId: z.uuid(), path: z.string().min(1).max(2048) }).strict() }).strict(),
  z.object({
    ...requestIdentity, operation: z.literal("skill.resource.write"),
    payload: z.object({ skillId: z.uuid(), kind: z.enum(["reference", "asset", "script", "test"]), path: z.string().min(1).max(2048), encoding: z.enum(["utf8", "base64"]), content: z.string().max(7_000_000) }).strict(),
  }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("skill.resource.remove"), payload: z.object({ skillId: z.uuid(), path: z.string().min(1).max(2048) }).strict() }).strict(),
  ...
  z.object({
    ...
    payload: z.object({ name: z.string().trim().min(1).max(120), skillId: z.uuid(), client: z.enum(["claude", "codex"]), preferredModel: z.string().max(200), outputDirectory: z.string().max(2048), inlineInstructions: z.string().max(100_000), executionMode: z.enum(["manual", "headless"]).default("manual"), headlessExecutablePath: z.string().max(4096).nullable().optional(), headlessAccessMode: z.enum(["read-only", "staged-write"]).default("read-only"), headlessMaxRuntimeMs: z.number().int().min(1_000).max(3_600_000).default(300_000) }).strict(),
  }).strict(),
  z.object({
    ...requestIdentity, operation: z.literal("routine.update"),
    payload: z.object({ routineId: z.uuid(), name: z.string().trim().min(1).max(120), skillId: z.uuid(), client: z.enum(["claude", "codex"]), preferredModel: z.string().max(200), outputDirectory: z.string().max(2048), inlineInstructions: z.string().max(100_000), executionMode: z.enum(["manual", "headless"]).default("manual"), headlessExecutablePath: z.string().max(4096).nullable().optional(), headlessAccessMode: z.enum(["read-only", "staged-write"]).default("read-only"), headlessMaxRuntimeMs: z.number().int().min(1_000).max(3_600_000).default(300_000) }).strict(),
  }).strict(),
  z.object({
  ...
  z.object({
    ...requestIdentity, operation: z.literal("routine.launch"),
    payload: z.object({ routineId: z.uuid(), objective: z.string().min(1).max(100_000), targetPaths: z.array(z.string().max(2048)).max(50), sources: z.array(z.object({ baseId: z.union([z.literal("private"), z.uuid()]), documentId: z.string().min(1).max(128) }).strict()).max(100), outputIds: z.array(z.uuid()).max(100).default([]) }).strict(),
  }).strict(),
  ...
  z.object({ ...requestIdentity, operation: z.literal("headless.discover"), payload: z.object({}).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("headless.list"), payload: z.object({}).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("headless.start"), payload: z.object({ provider: z.enum(["claude", "codex"]), executablePath: z.string().min(1).max(4096).refine((value) => value.startsWith("/"), "Path must be absolute"), prompt: z.string().trim().min(1).max(100_000), model: z.string().max(200).nullable().optional(), accessMode: z.enum(["read-only", "staged-write"]).default("read-only"), maxRuntimeMs: z.number().int().min(1_000).max(3_600_000).optional() }).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("headless.cancel"), payload: z.object({ runId: z.uuid() }).strict() }).strict(),
  z.object({ ...requestIdentity, operation: z.literal("headless.applyWriteback"), payload: z.object({ runId: z.uuid(), paths: z.array(z.string().min(1).max(2048)).min(1).max(200) }).strict() }).strict(),
...
export const IPC_CHANNELS = {
  ...
  artifactSearch: "voidra:artifact:search",
  ...
  artifactReview: "voidra:artifact:review",
  artifactRollback: "voidra:artifact:rollback",
  artifactConvertLegacy: "voidra:artifact:convert-legacy",
  artifactGrant: "voidra:artifact:grant",
  artifactRevoke: "voidra:artifact:revoke",
  artifactCapability: "voidra:artifact:capability",
  artifactWriteList: "voidra:artifact:write-list",
  artifactWriteApply: "voidra:artifact:write-apply",
  artifactWriteReject: "voidra:artifact:write-reject",
} as const;
